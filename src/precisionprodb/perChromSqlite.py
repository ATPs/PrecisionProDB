...
def build_parser():
    ...
    add_argument_set(
        ...
        overrides={
            'sqlite': {
                'flags': ('-q', '--sqlite'),
                'help': 'sqlite file file with CDS and exon annotations',
                'required': True,
            },
            'out': {
                'default': 'perChrom',
                'help': 'output prefix. two file will be output. One is the annotation for mutated transcripts, one is the protein sequences. {out}.aa_mutations.csv, {out}.mutated_protein.fa. default "perChrom" ',
            },
        },
    )
    return parser
...
def run_from_args(f):
    perchrom_sqlite = PerChrom_sqlite(file_sqlite = f.sqlite,
...
def main(argv=None):
    ...
    if argv is None and TEST:
        argv = ["-q", file_sqlite, "-m", file_mutations, "-t", str(threads), "-o", outprefix, "-c", chromosome, "-a", datatype]
