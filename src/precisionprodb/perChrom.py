...
def readExtraLargeMutationFile(file_mutations, columns_to_include=None, columns_to_drop=None, nrows=None):
    ...
    if columns_to_include is not None:
        columns = [col for col in columns if col in columns_to_include]
...
def parse_mutation(file_mutations, chromosome=None, columns_to_include=None, columns_to_drop=None, nrows=None):
    '''
    Parse mutation file.
...
def build_parser():
    ...
    add_argument_set(
        ...
        overrides={
            ...
            'protein': {'default': None, 'help': 'protein sequences in fasta format. It can be a gzip file.'},
            'gtf': {'required': True, 'help': 'gtf file with CDS and exon annotations'},
            'out': {
                'default': None,
                'help': 'output prefix. two file will be output. One is the annotation for mutated transcripts, one is the protein sequences. {out}.aa_mutations.csv, {out}.mutated_protein.fa. {out}.mutated_protein.fa only includes the proteins AA changes. ',
            },
        },
    )
    return parser
...
def run_from_args(f):
    perchrom = PerChrom(file_genome = f.genome,
                    file_gtf = f.gtf,
    ...
    print('run perChrom for chromosome', f.chromosome)
