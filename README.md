...
```bash
python Path_of_PrecisionProDB/src/PrecisionProDB.py -m Path_of_mutation_file -D GENCODE -o Prefix_of_output
```
...
```bash
python Path_of_PrecisionProDB/src/PrecisionProDB.py -m Path_of_mutation_file -D GENCODE -o Prefix_of_output -s Sample_name
```

### VCF with local gene annotation
If there is a local version of gene annotation from Ensembl, the command will be:
```bash
python Path_of_PrecisionProDB/src/PrecisionProDB.py -m Path_of_mutation_file -o Prefix_of_output -s Sample_name -g Ensembl_Genome -p Ensembl_protein -f Ensembl_gtf -a Ensembl_GTF
```
