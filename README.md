...
- [What's new in PrecisionProDB v2](#whats-new-in-precisionprodb-v2)
  - [PrecisionProDB online](#precisionprodb-online)
...
- [Usage Information](#usage-information)
  - [Typical usage](#typical-usage)
    ...
    - [VCF manifest for one-VCF-per-sample cohorts](#vcf-manifest-for-one-vcf-per-sample-cohorts)
...
* Supports variant file in text, VCF, or VCF manifest format.
...
```bash
python Path_of_PrecisionProDB/src/precisionprodb/PrecisionProDB.py -m Name_of_variant_file -D GENCODE -o Prefix_of_output
```
...
/path/to/sample_A.vcf.gz	default	sample_A
/path/to/sample_B.vcf.gz	default	sample_B
```

- `filepath` is required and must point to a `.vcf` or `.vcf.gz` file.
...
```bash
python Path_of_PrecisionProDB/src/precisionprodb/PrecisionProDB.py -m Name_of_variant_file -o Prefix_of_output -s Sample_name -g Ensembl_Genome -p Ensembl_protein -f Ensembl_gtf -a Ensembl_GTF
```
...
```bash
python Path_of_PrecisionProDB/src/precisionprodb/downloadHuman.py -d Ensembl -o Output_folder
```
...
```bash
python Path_of_PrecisionProDB/src/precisionprodb/PrecisionProDB.py -m Name_of_variant_file -o Prefix_of_output -s Sample_name -g TransDecoder_Genome -p TransDecoder_protein -f TransDecoder_gtf -a gtf
```
...
python ../src/precisionprodb/PrecisionProDB.py -m celline.vcf.gz -g GENCODE.genome.fa.gz -p GENCODE.protein.fa.gz -f GENCODE.gtf.gz -o vcf_variant
```
...
```bash
cd Path_of_PrecisionProDB/examples
python ../src/precisionprodb/PrecisionProDB.py -m gnomAD.variant.txt.gz -g GENCODE.genome.fa.gz -p GENCODE.protein.fa.gz -f GENCODE.gtf.gz -o text_variant
```
...
There are several files in the `src/precisionprodb` folder. Each of them were designed in a way that can be run independently. To get help, run

```bash
python Path_of_PrecisionProDB/src/precisionprodb/MODULE_NAME.py -h
```
where `MODULE_NAME` should be the files in the `src/precisionprodb` folder, without the `.py` extension.

To get help for the main program, run

```bash
python Path_of_PrecisionProDB/src/precisionprodb/PrecisionProDB.py -h
```
...
# Workflow
[PrecisionProDB workflow](./media/workflow.md)
